 ...
 */
public class GetInvitedGamesTask extends AsyncTask<Void, Void, List<GameData>>{
     ...
     */
    public GetInvitedGamesTask(Context context, Token token, ArrayAdapter adapter, FrameLayout progressLayout,
    ...
    protected List<GameData> doInBackground(Void... voids) {
        ...
        try {
            collection = gameEndpoint.getInvitedGames(loginToken).execute();
        } catch (IOException e) {
